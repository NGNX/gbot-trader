...
Бонус предоставляется в виде продления лицензии на 10 дней.
...
* **При работе без RSI**: (`RSI_PERIOD = 0`) установка ордеров на покупку происходит если цена приблизилась к **lowband** или стала ниже **lowband**. Ордера на продажу устанавливаются аналагично только по уровню **highband** или если цена больше него.
...
* **NOTIFICATION_ORDER_IS_EXECUTED** отвечает за уведомление об исполнении one orders. (Так же можно вкл/выкл через Telegram: `Нотис` -> `Исполнение one orders`)
...
**Внимание!** Если по какой-то причине Trailing Stop дал команду на установку ордера, а профитный ордер не установился, то в консоле будет уведомление об ошибке **Problem with Trailing Stop active**.
...
* При использовании Trailing Stop "авто-выход из пары" не работает. 
...
* **Изменить отступ** - изменяет отступ ордеров, первого ордера и процент профита.
...
* **Авто-выход из пары** - Установка бота на паузу после исполнения первого же профит ордера с установкой **0 профита** для скорейшего выхода. (Только для One orders).
